...
public class SolrSendTrademarkCase  {
    ...
    private SolrClient createNewSolrClient(SolrInfo solrInfo) {
        try {
            logger.info("server address:" + solrInfo.getSolrHome()+"bdycase");
            HttpSolrClient client = new HttpSolrClient(solrInfo.getSolrHome()+"bdycase");
    ...
    public ReturnInfo selectTradeMarkCase(GeneralCondition gcon, Integer departId,Integer custId,Integer agencyId,
    		TradeMarkCaseSolr tradeMarkCase,String startYear,String endYear, String queYear,
			SolrInfo solrInfo,
			ReturnInfo returnInfo) throws Exception {
		SolrClient client = createNewSolrClient(solrInfo);
    	try {
    		Map<String, String> queryMap = new HashMap<String, String>();
	    	queryMap.put("q", getQueryStringBySelectTradeMarkCase(gcon,departId,custId, agencyId, 
	        		   tradeMarkCase, startYear,  endYear,queYear));
    		...
    		returnInfo.setData(getTradeMarkCaseBySolrResults(response.getResults()));
	 ...
	 private String getQueryStringBySelectTradeMarkCase(GeneralCondition gcon, Integer departId, Integer custId, Integer agencyId,
			 TradeMarkCaseSolr tradeMarkCase, String startYear, String endYear, String queYear) {
		String queryList = "*:*";
    	if(custId != null) {
    		queryList = queryList+" AND custId:"+custId.toString();     		
    	}
    	if(agencyId != null) {
    		queryList = queryList+" AND agencyId:"+agencyId.toString(); 
    	}
    	if(departId != null) {
    		queryList = queryList+" AND departIdTree:"+departId.toString(); 
    	}
    	...
    	queryList = queryList+SolrUtil.GetSolrQueryByClass(tradeMarkCase, ignore);
		return queryList;
	}

	 
		private List<TradeMarkCase> getTradeMarkCaseBySolrResults(SolrDocumentList docs) {
			List<TradeMarkCase> tradeMarkCaselist = new ArrayList<TradeMarkCase> ();
			List<String> ignore = new ArrayList<String>();
			for(SolrDocument doc : docs) {
				TradeMarkCase tradeMarkCase = SolrUtil.SolrDocumentToClass(TradeMarkCase.class,doc,ignore);
				//trademark.setTmImage("/TmImage/"+trademark.getApplicantName().toString()+"/"+trademark.getRegNumber().toString()+".jpg");			
				tradeMarkCaselist.add(tradeMarkCase);
			}
			return tradeMarkCaselist;
		}
	
	 public void createDocs(SolrInfo solrInfo,SolrData trcaseDataList,List<SolrData> otherDataList) throws Exception {
        logger.info("======================add doc ===================");
        Collection<SolrInputDocument> docs = new ArrayList<SolrInputDocument>();
    	SolrClient client = createNewSolrClient(solrInfo);
        try {
        	docs = getSolrDocByCase(trcaseDataList, otherDataList);
	...
	private Collection<SolrInputDocument> getSolrDocByCase(SolrData trcaseList,List<SolrData> otherDataList) {
		Collection<SolrInputDocument> docs = new ArrayList<SolrInputDocument>();
		docs = getSolrDocbyTCaselist(trcaseList);
		for(SolrData otherData : otherDataList) {
			docs = changeSolrDocByOtherData(otherData,docs,"id");
		}
		return docs;
	}
	
	
    public Collection<SolrInputDocument> getSolrDocbyTCaselist(SolrData trcaseList) {
        Collection<SolrInputDocument> docs = new ArrayList<SolrInputDocument>();
        
		List<Map<String, Object>> trcaseMapList = trcaseList.getDataTable();
		for (Map<String, Object> trcaseMap:trcaseMapList) {
			SolrInputDocument doc = new SolrInputDocument();
			for (Map.Entry<String, Object> trcase : trcaseMap.entrySet()) {
				//if (trademark.getKey().equals("tmImage") ) continue;
				if(trcase.getValue() instanceof BigDecimal) {
					doc.addField(trcase.getKey(), ((BigDecimal)trcase.getValue()).doubleValue());
				} else {
					doc.addField(trcase.getKey(), trcase.getValue());
				}
			}
			docs.add(doc);			
		}
    			...
    			for(Map<String,Object> multMapValueMap:multMapValue) {
    				for(Map.Entry<String, Object> entry : multMapValueMap.entrySet()) {
    	        		...
    	        		if (entry.getKey().equals("userId") ) {
    	        			doc.addField("userIdTree", entry.getValue()); 
    	        			continue;
    	        		}
    	        		if (entry.getKey().equals("username") ) {
    	        			doc.addField("userNameTree", entry.getValue()); 
    	        			continue;
    	        		}
    ...
    public Object SelectTradeMarkCaseCount(SolrInfo solrInfo,Integer custId,Integer agencyId,Integer departId,Integer startYear,Integer endYear,String queYear,String otherque) throws Exception{
    	...
    	queryMap.put("rows", "0");
    	try {
    		SolrDocumentList solrDocumentList=SolrUtil.solrQuery(queryMap,client).getResults();
    		long numFount=solrDocumentList.getNumFound();
    		return numFount;
...
//			}
    ...
    public Object SelectTradeMarkCaseByList(SolrInfo solrInfo,Integer custId,Integer agencyId,Integer departId,Integer startYear,Integer endYear,List<String> fieldList,String queYear,boolean ignore0) throws Exception{
    	List<Map<String, Object>> retlist = new ArrayList<Map<String, Object>>();
    	SolrClient client = createNewSolrClient(solrInfo);
    	Map<String, String> queryMap = new HashMap<String, String>();
    	String queryList = "*:*";
